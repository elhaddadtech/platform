...
function TestReportG({data,setter}) {
    ...
    const faculty =()=>{
      setGroups(filterGroup(data))
        setStu(groups)
        const dtg = groups.map((group)=>{return data.filter(element => {  return(  ([...element.Group.split('_')][0].trim()) == group.trim()) })})
        setDtgroup(dtg)
        console.log('grs',dtg)
        const tgrs = dtg.map((d)=>{return stat(d)})
        console.log('t',tgrs)
        setTotales(tgrs)
        const arr=totales.map((totale)=>{return totale[0]})
        let i = 0,sum=0,sum2=0
        for(i=0;i<arr.length;i++){
          sum+=arr[i]
        }
        const arr2=totales.map((totale)=>{return totale[2]})
        for(i=0;i<arr.length;i++){
          sum2+=arr2[i]
        }
        const a=[sum,sum2]
        ...
        const res = dtg.map((d)=>{return Sc(d)})
  ...
  return (
    <div>
      <div>
        <select className="select select-success w-full max-w-md mt-2" defaultValue={'false'} onChange={(e)=>{setter(e.target.value);setGroup(e.target.value)}}>
          ...
          <tr>
            <th>
              {
                groups?.map((group,i)=>{
                  ...
                  return (
                    ...
                    <tr key={i}>
                      <td class="my-2" ><input type="checkbox"  key={i} className="" onChange={(e)=>{setCheck(!check);if(e.target.checked){console.log("stu",stu,e.target.checked)}}}/> {group==""?"Sans Group":group}</td>
                    </tr>
                  );
            ...
            <th>
              {totales?.map((t,i) => {
                return (
                  <tr key={i}>
                    <td>{t[2]!==0?((rs[i]/t[2])*100).toFixed():0}%</td>
                  </tr>
                );
              })}
            </th>
